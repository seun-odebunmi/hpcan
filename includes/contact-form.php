...
if(isset($_POST['email'])) {
	...
	$fields = array(
		0 => array(
			'text' => 'Name',
			'val' => $_POST['firstname']." ".$_POST['lastname']
		),
		1 => array(
			'text' => 'Email address',
			'val' => $_POST['email']
    ...
    if ($arrResult['response'] == 'success'){
        ...
        header('Location: '.$contactURL.'?email=sent-successfully');
        
    }else if ($arrResult['response'] == 'error'){
        
        header('Location: '.$contactURL.'?email=not-sent');
    }
